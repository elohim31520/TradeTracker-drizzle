...
export async function getTodayStocks(): Promise<StockPrice[]> {
...
export async function getMarketBreadth(): Promise<number> {
    const stocks = await getTodayStocks();
...
export async function getStockDayChgSorted() {
    const stocks = await getTodayStocks();
    return stocks
        .map((stock) => ({
            ...stock,
