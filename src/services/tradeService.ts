...
export async function create(data: NewTrade, tx: any = db): Promise<Trade[]> {
	return await tx.insert(stockTrades).values(data).returning()
}

export async function bulkCreate(data: NewTrade[], tx: any = db): Promise<Trade[]> {
	return await tx.insert(stockTrades).values(data).returning()
}

export async function getAll({ userId, page, size }: GetAllTradesParams): Promise<PaginatedTrades> {
...
export async function getById(id: number): Promise<Trade | null> {
	const [trade] = await db.select().from(stockTrades).where(eq(stockTrades.id, id)).limit(1)
	return trade || null
}

export async function update(id: number, data: Partial<NewTrade>): Promise<Trade> {
...
export async function deleteTrade(id: number, userId: string): Promise<void> {
